...
RELOADED_ROOT="${MY_DIR}/../../.."

HELPER="$RELOADED_ROOT/vendor/reloaded/build/tools/extract_utils.sh"
if [ ! -f "$HELPER" ]; then
...
setup_vendor "${DEVICE}" "${VENDOR}" "${RELOADED_ROOT}" false "${CLEAN_VENDOR}"

extract "${MY_DIR}"/proprietary-files.txt "${SRC}" \
        "${KANG}" --section "${SECTION}"

DEVICE_BLOB_ROOT="${RELOADED_ROOT}"/vendor/"${VENDOR}"/"${DEVICE}"/proprietary
